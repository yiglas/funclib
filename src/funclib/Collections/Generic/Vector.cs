...
namespace funclib.Collections
{
        ...
        IReduceKV<int, T>
    {
        ...
        internal static readonly VectorNode<T> EmptyNode = new VectorNode<T>(NoEdit, new object[32]);
        public static readonly IVector<T> EMPTY = new Vector<T>(0, 5, EmptyNode, new object[0]);
        ...
        internal Vector(int count, int shift, VectorNode<T> root, T[] tail)
        {
            Count = count;
            Shift = shift;
            Root = root;
        ...
        {
            if (init.Length <= 32)
                return new Vector<T>(init.Length, 5, EmptyNode, init);
        ...
        public override IVector<T> Assoc(int i, T val)
        {
            if (i >= 0 && i < Count)
            {
                if (i >= TailOff())
                {
                    var newTail = new T[this.Tail.Length];
            ...
            {
                var ret = new VectorNode<T>(node.Edit, (T[])node.Array.Clone());
                if (level == 0)
        ...
        public override IVector<T> Cons(T o)
        {
            if (Count - TailOff() < 32)
            {
                var newTail = new T[Tail.Length + 1];
            ...
            return new Vector<T>(Count + 1, newShift, newRoot, new T[] { o });
        }
        public override ICollection<T> Empty() => EMPTY;
        public override IStack<T> Pop()
        {
            ...
            if (Count - TailOff() > 1)
            {
                newTail = new T[this.Tail.Length - 1];
        ...
        public override ISeq<T> Seq() => Count == 0 ? null : new ChunkedSeq<T>(this, 0, 0);
        ...
        VectorNode<T> PushTail(int level, VectorNode<T> parent, VectorNode<T> tailNode)
        {
            ...
            var ret = new VectorNode<T>(parent.Edit, (T[])parent.Array.Clone());
        ...
        VectorNode<T> PopTail(int level, VectorNode<T> node)
        {
            ...
            if (level > 5)
            {
                ...
                {
                    var ret = new VectorNode<T>(this.Root.Edit, (T[])node.Array.Clone());
        ...
        public TResult ReduceKV<TInit, TResult>(Func<TInit, int, T, TResult> f, TInit init)
        {
            int step = 0;
            for (int i = 0; i < Count; i += step)
            {
