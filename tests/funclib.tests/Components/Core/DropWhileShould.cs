...
namespace funclib.Tests.Components.Core
{
    public class DropWhileShould
    {
        ...
        public void DropWhile_should_return_a_list_with_item_after_first_false()
        {
            ...
            var actual = toArray(dropWhile(new IsNeg(), new Vector().Invoke(-1, -2, -6, -7, 1, 2, 4, -5, -6, 0, 1)));
        ...
        {
            var expected = list();
            var actual = toArray(dropWhile(partial(new IsGreaterThan(), 10), new Vector().Invoke(1, 2, 3, 2, 1)));
